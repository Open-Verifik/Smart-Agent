...
export class SettingsService {
     ...
     */
    updateStaff(staffId: string, staffData: Partial<StaffMember>): Observable<StaffResponse> {
        if (!this.accessToken) {
            return throwError(() => new Error('No access token available'));
        }
    ...
    /**
     * Active SmartAccess plan for the client (if any).
     *
     * API: GET /v2/client-smart-access-plans?where_client={clientId}&where_status=active&findOne=1
     */
    getSmartAccessPlan(clientId: string): Observable<any> {
        if (!this.accessToken) {
            return throwError(() => new Error('No access token available'));
        }

        const url = `${this.apiUrl}/v2/client-smart-access-plans`;
        const params = {
            where_client: clientId,
            where_status: 'active',
            findOne: '1',
        ...
        return this._httpWrapper.sendRequest('get', url, params).pipe(
            catchError((error) => {
                console.error('[SettingsService] Get SmartAccess plan failed:', error);
                return throwError(() => error);
            })
        );
    }
    ...
    /**
     * Active SmartEnroll plan for the client (if any).
     *
     * API: GET /v2/client-smart-enroll-plans?where_client={clientId}&where_status=active&findOne=1
     */
    getSmartEnrollPlan(clientId: string): Observable<any> {
        if (!this.accessToken) return throwError(() => new Error('No access token available'));

        const url = `${this.apiUrl}/v2/client-smart-enroll-plans`;
