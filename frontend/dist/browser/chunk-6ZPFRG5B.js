...
var MarkdownPipe = class _MarkdownPipe {
  ...
  transform(value) {
    ...
    try {
      const raw = d(value, { async: false });
      if (this._isPromise(raw)) {
        return this.sanitizer.bypassSecurityTrustHtml(this._plainParagraph(value));
      }
      const html = typeof raw === "string" ? raw : String(raw ?? "");
      const withDocBase = this._prefixRootRelativeDocLinks(html, environment.documentationBaseUrl);
   ...
   */
  _prefixRootRelativeDocLinks(html, baseUrl) {
    if (!baseUrl?.trim()) {
      return html;
    }
    const base = baseUrl.replace(/\/$/, "");
    return html.replace(/(href=)(["'])(\/[^"']*)\2/gi, (match, p, q2, path) => {
      if (path.startsWith("//")) {
        return match;
      }
      return `${p}${q2}${base}${path}${q2}`;
    });
  }
  _plainParagraph(text) {
...
//# sourceMappingURL=chunk-6ZPFRG5B.js.map
